...
def create_app(model_names):
    ...
    async def clear_distributed_memory():
        ...
        try:
            logger.info("Starting comprehensive distributed memory cleanup...")
            
            # Step 1: Clear all named actors (trained models)
            all_actors = ray.util.list_named_actors()
            cleared_actors = 0
            failed_actors = []
            
            logger.info(f"Found {len(all_actors)} named actors to clear")
            ...
            for actor_name in all_actors:
                try:
                    ...
                    ray.kill(actor)
                    cleared_actors += 1
                    logger.info(f"Successfully cleared actor: {actor_name}")
                except Exception as e:
                    failed_actors.append(actor_name)
                    logger.warning(f"Could not clear actor {actor_name}: {e}")
            
            # Step 2: Clear object store data through data manager
            data_manager = get_data_manager()
            files_cleared = 0
            
            try:
                # Get count before clearing
                files_list = data_manager.list_files()
                files_cleared = len(files_list)
                logger.info(f"Found {files_cleared} files in object store: {files_list}")
                
                # Clear all stored datasets
                success = data_manager.clear_all()
                if success:
                    logger.info(f"Successfully cleared {files_cleared} datasets from object store")
                else:
                    logger.warning("Failed to clear object store through data manager")
                    files_cleared = 0
            except Exception as e:
                logger.warning(f"Could not clear object store through data manager: {e}")
                files_cleared = 0
            
            # Step 3: Force Ray object store cleanup
            try:
                # Get object store statistics before cleanup
                try:
                    object_store_stats = ray._private.internal_api.memory_summary(stats_only=True)
                    logger.info(f"Object store stats before cleanup: {object_store_stats}")
                except Exception:
                    logger.info("Could not get object store stats")
                
                # Force garbage collection on all nodes
                import gc
                gc.collect()
                
                # Try multiple methods to trigger Ray's internal garbage collection
                try:
                    ...
                    logger.warning(f"Could not trigger Ray GC method 1: {gc_e}")
                    
                # Alternative method: try to clear object store through worker processes
                try:
                    @ray.remote
                    ...
                    # Run on all available workers
                    futures = [clear_worker_memory.remote() for _ in range(min(4, ray.available_resources().get('CPU', 1)))]
                    ray.get(futures, timeout=10)
                    logger.info("Triggered garbage collection on worker nodes")
                except Exception as worker_gc_e:
                    logger.warning(f"Could not trigger worker GC: {worker_gc_e}")
                
            except Exception as e:
                logger.warning(f"Could not trigger Ray object store cleanup: {e}")
            
            # Step 4: Reset the global data manager instance
            try:
                global _data_manager
                from load_data import _data_manager
                _data_manager = None
                logger.info("Reset global data manager instance")
            except Exception as e:
                logger.warning(f"Could not reset data manager: {e}")
            ...
            # Verify cleanup
            remaining_actors = ray.util.list_named_actors()
            remaining_files = get_data_manager().list_files()
            
            logger.info(f"Memory cleanup completed. Cleared {cleared_actors} actors, {files_cleared} datasets")
            logger.info(f"Remaining: {len(remaining_actors)} actors, {len(remaining_files)} files")
            
            return {
                "status": "success",
                "actors_cleared": cleared_actors,
                "actors_failed": len(failed_actors),
                "failed_actors": failed_actors,
                "datasets_cleared": files_cleared,
                "remaining_actors": len(remaining_actors),
                "remaining_files": len(remaining_files),
                "message": f"Distributed memory cleared successfully. {cleared_actors} actors and {files_cleared} datasets removed."
            }
